export async function compileJava(code, filename) {
  ...
  try {
    if (!code || typeof code !== 'string') {
      return {
        success: false,
        output: 'Error: No code provided or invalid code format',
        error: 'Invalid code',
    ...
    const className = classNameMatch ? classNameMatch[1] : (filename ? filename.replace('.java', '') : 'Main');
    ...
    console.error('CompileJava error:', error);
    return {
      success: false,
